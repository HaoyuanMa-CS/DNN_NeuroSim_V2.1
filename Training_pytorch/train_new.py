...
parser.add_argument('--nlsLTPc1', default=1)
parser.add_argument('--nlsLTPc2', default=-1.5)
parser.add_argument('--nlsLTDc1', default=1)
parser.add_argument('--nlsLTDc2', default=-3)
...
args.nlsLTPc1 = 0.7738
args.nlsLTPc2 = 2.0510
args.nlsLTDc1 = 0.1727
args.nlsLTDc2 = 3.0872       # (1-exp(-c1*x/exp(-c2*x)))
args.epochs = 1
# momentum
gamma = 0.9
alpha = 0.1
...
try:
    ...
    for layer in list(model.parameters())[::-1]:
        ...
        NLS_LTP_C1[k] = (torch.ones_like(layer)*args.nlsLTPc1+d2dVariation).cpu().numpy()
        NLS_LTP_C2[k] = (torch.ones_like(layer)*args.nlsLTPc2+d2dVariation).cpu().numpy()
        NLS_LTD_C1[k] = (torch.ones_like(layer)*args.nlsLTDc1+d2dVariation).cpu().numpy()
        NLS_LTD_C2[k] = (torch.ones_like(layer)*args.nlsLTDc2+d2dVariation).cpu().numpy()
        k=k+1
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            for name, param in list(model.named_parameters())[::-1]:
                ...
                param.grad.data = wage_quantizer.QG_NLS(param.data,args.wl_weight,param.grad.data,args.wl_grad,grad_scale,
                            torch.from_numpy(NLS_LTP_C1[j]).cuda(), torch.from_numpy(NLS_LTP_C2[j]).cuda(), 
                            torch.from_numpy(NLS_LTD_C1[j]).cuda(), torch.from_numpy(NLS_LTD_C2[j]).cuda(),
                            args.max_level, args.max_level)
                j=j+1
